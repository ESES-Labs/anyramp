...
export const openApiDoc = {
  ...
  info: {
    ...
    description:
      ...
      'buyer pays QRIS → `POST /orders/{id}/settle-real` (poll: confirm payment + zkTLS prove) → ' +
      '`POST /orders/{id}/settle` (buyer signs, USDC to their wallet) or `/settle/auto` (server) → USDC released.\n\n' +
  ...
  paths: {
    ...
    '/orders/{id}/settle-real': {
      post: {
        tags: ['settlement'],
        summary: 'Poll while showing the QR: confirm payment + prove (returns current order)',
        parameters: [idParam],
        responses: { 200: jsonResp('current order', { type: 'object' }) },
      },
    },
    '/orders/{id}/prove': {
      post: {
    ...
    '/orders/{id}/settle/auto': {
      post: {
        tags: ['settlement'],
        summary: 'Server auto-submits fulfill (fallback, buyer == submitter)',
        parameters: [idParam],
