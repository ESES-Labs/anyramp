...
export async function generateProof(orderId: string, amount: number): Promise<ReclaimProofLike> {
  ...
  return await new Promise<ReclaimProofLike>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      clearTimeout(timer);
      // prove.mjs delimits the proof with a sentinel (its stdout is otherwise polluted
      // by the attestor-core logger), so slice from there rather than parse the whole stream.
      const marker = '__ANYRAMP_PROOF__';
      const at = out.lastIndexOf(marker);
      if (at === -1) {
        return reject(new Error(`prove failed: ${err.slice(-500) || out.slice(-500) || 'exit ' + code}`));
      }
      try {
        resolve(JSON.parse(out.slice(at + marker.length)) as ReclaimProofLike);
      } catch {
