...
// Poll-driven real settlement: the frontend hits this every few seconds while showing
// the QR. It confirms payment via transactiondetail and, once paid, kicks off the
// prove→settle pipeline in the background. Always returns the current order so the
// client can drive its UI off `status` (created → proving → fulfilled + txHash).
orders.post('/:id/settle-real', async (c) => {
  const id = c.req.param('id');
  const order = await store.getOrder(id);
  if (!order) throw new HTTPException(404, { message: 'not found' });
  if (order.status === 'created' || order.status === 'paid_detected') {
    void runSettlement(id); // self-guards against duplicates; no-op until payment lands
  }
...
orders.post('/:id/submit', zValidator('json', z.object({ signedXdr: z.string().min(1) })), async (c) => {
  ...
  const updated = await store.updateOrder(order.orderId, { status: 'fulfilled', txHash: hash });
